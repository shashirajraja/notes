		...
		try {
			...
			/*
			  CREATE OR REPLACE PROCEDURE CREATE_ACCOUNT
				(id number, name varchar2, email varchar2, mob number, gender varchar2, dob varchar2 ,
				   password varchar2 ,accno number, balance number, type varchar2)
				IS
				
				BEGIN
				 INSERT INTO CUSTOMER VALUES(id, name, email, mob, gender, dob, password);
				 INSERT INTO BANK VALUES(accno, balance, type);
				END;
			 */
			CallableStatement cs = connection.prepareCall("{call CREATE_ACCOUNT(?,?,?,?,?,?,?,?,?,?)}");
