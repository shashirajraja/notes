...
void merge(int* arr, int lb, int mid ,int ub){
	...
	while(i < n1) { //when arr2 is completely visited but the elements are left in arr1 to be visited
		arr[k] = arr1[i];
		i++;
		k++;
	} 
	
	while (j < n2){ //when arr1 is completely visited but the elements are left in arr2 to be visited
		arr[k] = arr2[j];
		j++;
		k++;
	}
